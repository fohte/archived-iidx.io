    ...
    context 'with known musics' do
      let!(:music) do
        create(
          :music,
          maps: [
            build(:map, play_style: :sp, difficulty: :normal, num_notes: 771),
            build(:map, play_style: :sp, difficulty: :hyper, num_notes: 1116),
            build(:map, play_style: :sp, difficulty: :another, num_notes: 1301),
          ],
          series: :iidx_red,
          title: 'gigadelic',
          csv_title: 'gigadelic',
    ...
    context 'when results are played in the 段位認定 mode only' do
      let!(:music) do
        create(
          :music,
          maps: [
            build(:map, play_style: :sp, difficulty: :hyper, num_notes: 1078),
          ],
          series: :spada,
          title: '疾風迅雷',
          csv_title: '疾風迅雷',
